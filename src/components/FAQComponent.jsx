...
const FAQComponent = ({ faqData, containerHeight, title}) => {
    ...
    return (
        <div className="mx-auto max-w-[1440px]">
            <div className="bg-[#2E3555] mx-20 py-20 px-8 mt-28 mb-32 rounded-[40px]">
                <div className='text-white flex justify-between items-center gap-8'>
                    ...
                    <div className='flex-[0.35] flex flex-col justify-between h-full'>
                        <h1 className="text-4xl w-60 font-bold leading-normal">
                            {title}
                        </h1>
                        <div style={{ height: containerHeight }}></div>
                        
                        <div className="mt-8 bg-[#171E38] p-8 rounded-2xl flex flex-col gap-6">
                            <p className='text-2xl font-medium font-inter'>Still have a question?</p>
                            ...
                            <img src={Group} alt="Group of support team members" className="w-[336px] h-[75px] rounded-lg" />
                        </div>
                    </div>
                        ...
                        <Accordion type="single" collapsible>
                            {faqData.map((faq, index) => (
                                <AccordionItem
                                    value={`item-${index}`}
                                    key={index}
                                    className="mb-4 border-none" // Ensure no borders on the item
                                >
                                    <AccordionTrigger
